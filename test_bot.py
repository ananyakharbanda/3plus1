...
class Bot:
    ...
    def _cleanup_unwanted(self, pos, tickers, wallet):
        ...
        for pair, info in list(pos.items()):
            ...
            if pnl_pct >= 0:
                self.L.info(f"  CLEANUP: selling {coin} (unwanted, +{pnl_pct:.2f}%)")
                avail = wallet.get(coin, {}).get("Free", 0)
                if avail > 0:
                    prec = self.pair_info.get(pair, {}).get("AmountPrecision", 2)
                    ...
                    if sell_qty > 0:
                        ...
                        if ok:
                            ...
                            trade_pnl = round((fpx - avg) * sell_qty, 2) if avg > 0 else 0
                            self.realized_pnl += trade_pnl
                            self.L.info(f"  SOLD {sell_qty} {coin} @ ${fpx:,.2f} P&L:${trade_pnl:+,.2f} (cleanup)")
    ...
    def tick(self):
        ...
        self.L.info(
            ...
            f"R:${self.realized_pnl:+,.0f} DD:{dd:.1%} Cash:{cash_pct:.0%} [{pos_str}] → [{tgt_str}]"
        )
